...
public class EfficientDocument extends Document {
	...
	private void processText()
	{
		...
		for(String c:tokens) {
			if(isWord(c)==true) {
				numWords++;
				numSyllables = numSyllables + countSyllables(c);
			}
			else {
				numSentences++;
			}
		...
		if(tokenSize >0) {
			String lastWord = tokens.get(tokenSize-1);
		...
		/*int length = requiredText.length();
		char lastChar = requiredText.charAt(length-1);
